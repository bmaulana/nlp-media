...
From the year-sentiment two-dimensional histogram plot, it would appear that there was a trend of declining sentiment scores over time. 
...
The pipeline also extracted the term frequency of all terms that matches a key term's lemma, from all news articles (refer to section \ref{des-matching} for details).
...
With this approach, the year-on-year average term occurrence trends of each key term in the `disabled' topic were measured and plotted in line graphs, one for each key term:
