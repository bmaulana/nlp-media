...
\begin{itemize}
	...
	\item vivekn's 'sentiment' repository, which provides an enhanced Na\"{i}ve Bayes classifier model trained on a publicly available dataset of 25,000 highly-polar movie reviews from the Internet Movie Database (IMDb) using bigrams and trigrams as features, an implementation of \cite{narayanan2013fast}. 
...
\end{itemize}
(Section \ref{tc-sentiment} has more details on the implementation details of these repositories/libraries)
...
\end{lstlisting}
The JSON objects of the sample is then output to a text file, where a user can manually change the 'label' fields to either '+' (positive), '-' (negative), 'n' (neutral), or 'o' (off-topic) to be read by the second script.
...
\begin{itemize}
	...
	\item False positive: count of positive-labelled sentences with sentiment score $\le$ 0.0
	\item True negative: count of negative-labelled sentences with sentiment score $\le$ 0.0
	\item False negative: count of negative-labelled sentences with sentiment score $>$\textbf{} 0.0
...
\ref{des-sentiment-comparison}
% results for how open-source sentiment scorers compare, plots, confusion matrices, accuracy, etc.

% Mention that 1) results from 180-article sample indicate VADER outperforms OpenAI in the domain (but not conclusively) 
% and 2) After manually analysing the plots generated by two versions of the pipeline (one using VADER and one using OpenAI), it is clear that VADER is more consistent in showing trend lines: VADER's line is 'smoother' and OpenAI's line has higher randomness with the same moving average coefficient (counting past 500 articles in both cases)
