...
Research into the representation of groups (e.g. women, youth) in the news media was common across different research fields, from social sciences to computer science.
...
This pipeline was then used in an experiment to collect articles from three major British online news publishers and showed trends regarding how the terms used and sentiment in news articles varies over time and between different publishers when reporting news related to disability or people with disabilities.
...
While the results showed the promise of performing quantitative analysis upon large bodies of literature, and specifically in the media representation domain, it is highly recommended that future approaches to this media analysis problem improve upon this work by training a domain-specific filter and sentiment scorer with labelled data to improve the accuracy, and thus consistency, of sentiment scoring.
...
In this research, a sample of 305,185 news articles (48,967 after filtering off-topic articles) from British online news sources was used.
...
When only considering articles from the same source, the moving average sentiment score stays roughly consistent.
The decreasing trend in `all' (shown above) was likely better attributed to the gradually decreasing proportion of Guardian articles and the gradually increasing proportion of Daily Mail articles in the dataset.
...
Terms were also converted to lower case so that capitalisation was ignored, thus e.g. `Disability' and `disability' are equivalent.
...
With this approach, the year-on-year average term occurrence trends of each key term in the `disabled' topic was plotted in line graphs, one for each key term:
