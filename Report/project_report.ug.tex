...
The computational models used in NLP range from simple rule-based models (e.g. splitting a sentence on whitespace to tokenise words) to statistical machine learning and deep learning models \cite{awesome-sentiment-analysis}. 
...
Analyses of news media in its various forms (print, online, etc.) is a popular research method, as the media provides an overview of the prevailing society's conceptions or views regarding a theme or topic, which can be quantified.
...
Furthermore, computational pipelines for data collection and scoring articles are more objective and reproducible than manual methods, which may vary due to each researcher's individual biases.
...
The Mann-Whitney U Test \cite{mann1947test} is a statistical metric that measures the null hypothesis that `given a randomly-selected value from a distribution, and another randomly-selected value from another distribution, the first value is equally likely to be less or greater than the second value'.
...
Then, three two-dimensional histograms that show the distribution of articles for:
\begin{itemize}
	\item The number of news articles for each year (X) and source (Y) in the dataset.
...
Then, for each term, a measure of average term occurrence, which measures the expected number of occurrences of a term in an article for a given year, is computed for each year between 2000 and 2018.
