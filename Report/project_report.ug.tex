...
For example, in \cite{Media reports on disability} it was shown that the Canadian press was more likely to name individuals with disability and use appropriate labelling than the Israeli press.
...
The topics brought up include the public's knowledge, attitudes, and beliefs about intellectual disability, as well as varying for socio-demographic characteristics, cross-cultural comparisons, and the effects of interventions.
...
A study has attempted to extract features (such as n-grams and part-of-speech tags) using natural language processing to classify racist and sexist posts in social media, providing annotations to 6,909 tweets \cite{Are You a Racist or Am I Seeing Things?}.
Another study explored potential linguistic markers of schizophrenia in social media from a dataset of 174 users with self-reported schizophrenia and up to 3,200 tweets per user, and control users in a 50:50 split \cite{Quantifying the Language of Schizophrenia in Social Media}.
They used a support vector machine (SVM) approach over features based on lexicon-based approaches (i.e. a list of mental health related keywords), latent dirichlet allocation (LDA), Brown clustering, character n-grams, and perplexity.
...
The sources used for this literature review are: % todo
\begin{itemize}
