...
** 2018 data is incomplete and would only include articles up to (approximately) late-March/early-April.
...
However, the uneven sample size (i.e. more Guardian articles vs Daily Mail or Daily Express articles over the full dataset) makes this visual representation hard to compare. The violin and box-and-whiskers plot show a better representation of this source-sentiment data:
...
Furthermore, the violin plot component show that the Guardian's distribution is more 'compact' around the mean (std. dev. = 0.363), the Mail has a slightly higher variance of sentiment scores (std. dev. = 0.386), and the Express has the highest variance of sentiment scores (std. dev. = 0.447). A version that shows violin plots for each 2-year period was also plotted:
