...
An advantage of NLP is that machines can process vast bodies of human-created literature (books, articles, posts, e-mails, messages, etc.) much faster than humans can, processing thousands of text documents per second. 
This allows for high-level quantitative analyses of thousands or millions of text documents from a vast corpora to be feasible, which can uncover information previously inaccessible by manually reading only a small sample of documents and generalising from there.
This level of quantitative analysis can uncover trends and patterns from a text corpus, such as ``how does the popularity of the term `mentally ill' increase or decrease year-on-year in British news media?'' 
...
This was achieved by analysing trends for $n$-gram (terms, or words/phrases) frequency and named entities in text.

More specifically in the domain of media representation of particular groups of people, several researchers have attempted to use features extracted using NLP to perform computational analytics of text, mainly from social media.
For example, a tool to classify racist and sexist posts in social media was developed by using NLP to extract $n$-grams and part-of-speech tags from text posts \cite{waseem2016you}.
However, there are still a research gap in this area, especially for applying NLP for news articles, in the context of specific groups such as people with disabilities.
% Just give a short summary, expand this later in Chapter 2

% Why is it interesting & what the challenges are
The representation of specific groups, such as people with disabilities, has been a popular research theme for psychologists, sociologists, and others.
...
Applying computational NLP to this field would allow the possibility of discovering higher-level trends, by computationally analysing a much larger sample of articles and identify trends by varying independent variables such as year of publication and publisher.
...
More specifically, to collect news articles relating to people with disabilities in British online media, and perform analyses using NLP at scale to identify trends such as term popularity and variations in positive/negative sentiment.

This project's goals was to achieve the stated aim by developing a computational pipeline capable of performing analysis on online news media in full, from data collection to analysis and visualisation.
Given a list of topics related to disabilities, each topic consisting of key terms and query terms;
this pipeline accomplishes the task of web crawling and scraping web sites to collect a dataset of news articles;
...
and producing relevant visualisations and statistical analyses to show trends as evidence of this application of NLP.
This pipeline will be available open source on GitHub (\url{https://github.com/bmaulana/nlp-media})

The main NLP techniques that was relevant for this project were:
...
term ($n$-gram) frequency, to quantitatively count the occurrence of words and phrases (sequences of words) in an article, which acted as a measure to compare articles;
sentiment analysis, to produce a `sentiment score' of news articles that correspond to its perceived positive/negative view, which also acted as a measure to compare articles.
These techniques were implemented by utilising open-source NLP implementations, developed by the research community.
...
Results indicated that a relatively simpler rule-based model (VADER \cite{VADER}) outperformed more complex supervised machine learning or neural network models trained on other domains (such as tweets and IMDb/Amazon reviews), which proved to be less generalisable for this domain.

This project proved the feasibility of using NLP-based technologies to derive trends from a large corpora of online news articles relating to disabilities or people with disabilities.
