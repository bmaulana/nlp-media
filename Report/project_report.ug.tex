...
The results of the experiment showed that it is feasible to derive trends from a large corpora of news articles using a computational pipeline implementing natural language processing technologies, despite the limitations of our sentiment scorer.
...
This experiment showed that it is feasible to apply computational techniques using natural language processing to discover trends and form statistically-significant conclusions from a large dataset of news articles in this domain.
This could have a substantial impact on how research should be conducted for similar studies on the media's representation of specific groups of people.
Also, a domain-specific supervised model for filtering and sentiment scoring (trained on an adequately-sized labelled dataset of same-domain news articles) would likely improve the accuracy of these tasks, which should derive clearer, more consistent trends and more significant results.   
...
Apart from sentiment score analyses, natural language processing can also be applied to extract various other syntax-based features and analyse trends based on them.
Features based on counting words (term frequency) are a mainstay of natural language processing research.
Apart from sentiment scores, the pipeline also extracts the term frequency of all terms that matches a key term's lemma from each news article (refer to section \ref{des-matching} for details).
...
This experiment showed that it is feasible to apply natural language processing technologies to discover trends on a large dataset of news articles in this domain, which achieved the primary aim of this project.
