...
Results from preliminary runs of the experiments indicate that data collection took approximately 0.61 seconds per article for the Daily Express, 0.70 seconds per article for the Daily Mail, and 0.20 seconds per article for the Guardian. %todo re-run with home internet connection
...
% todo show comparison of 'disabled' moving average plot (VADER vs OpenAI)
\noindent
...
We also show a slight 'drop' in the sentiment distribution (or an increase of news articles with negative sentiment scores) around 2016.
...
% todo do year on year analysis on disabled topic
...
Apart from sentiment scores, our pipeline also extracts the term frequency of all terms that matches a key term's lemma from each news article (refer to section \ref{des-matching} for details).
...
While the data is less consistent for Daily Express and Daily Mail, this may also be caused by a lack of data for these sources before 2010, and a smaller overall sample size.
